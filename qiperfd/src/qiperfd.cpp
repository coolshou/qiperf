...
void QIperfd::clear()
{
    ...
    debug("iperfserver:" + QString::number(m_iperfwserver.count()) + " threads:" + QString::number(m_thserver.count()) + " iperfclient:" + QString::number(m_iperfworkers.count()) + " thread:" + QString::number(m_threads.count()));
}
...
{
    QString tp_data = QString(CMD_IPERF_TP_DATA) + ":" + QString::number(idx) + ":" + sInterval + ":" + data;
...
{
    QString s = QString(CMD_IPERF_EXTEND_WAIT) + ":" + QString::number(refrow) + ":" + QString::number(iwait) + ":" + QString::number(exitCode) + ":" + QString::number(restarttimes);
...
bool QIperfd::deleteScheduledTask(const QString &taskName)
{
    ...
    try
    {
        ...
        if (FAILED(hr))
            _com_issue_error(hr); // Will throw if task not found
...
void QIperfd::onWSactMessage(QString msg, QHostAddress fromAddr, quint16 fromPort)
{
    ...
    else if (act.startsWith(CMD_SSH_ADD))
    {
        ...
        if (d.length() == 7)
        {
            ...
            debug("idx:" + idx + " sshTarget:" + sshTarget + " sshPort:" + sshPort + " username:" + username + " password:" + password + " privateKeyFile:" + privateKeyFile + " timeout:" + QString::number(timeout), 5);
...
void QIperfd::runRequest(QString refid, QString from, QString reqcmd, QString cmds)
{
    qDebug() << "runRequest: refid:" << refid
             ...
             << " exec cmd:" << cmds;
    QString rpcmd = "";
    qDebug() << "jiocmdRespObj:" << jiocmdRespObj;
...
QString QIperfd::parserResponse(QString rpcmd, QString data)
{
    ...
    if (jiocmdObj.contains(rpcmd))
    {
        ...
        QMap<QString, QRegularExpression> regexMap;
        QJsonObject pasObj = jiocmdObj.value(rpcmd).toObject();
        for (const QString &key : pasObj.keys())
        {
            QString reg = pasObj.value(key).toString();
            // Build and compile the pattern once per key
            QString pattern = QString("%1%2").arg(QRegularExpression::escape(key), reg);
            regexMap.insert(key, QRegularExpression(pattern));
        }
