...
#include <QRegExp>
#include <QRegExpValidator> // below 5.15
#else
...
#else
    QRegularExpression regExpIP("localhost|((25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])[\\.]){3}(25[0-5]|2[0-4][0-9]|1[0-9][0-9]|[1-9][0-9]|[0-9])");
    QRegularExpression regExpNetPort("((6553[0-5])|[655[0-2][0-9]|65[0-4][0-9]{2}|6[0-4][0-9]{3}|[1-5][0-9]{4}|[1-9][0-9]{3}|[1-9][0-9]{2}|[1-9][0-9]|[0-9])");
    QValidator  *pReg = new QRegularExpressionValidator(regExpIP, this);
#endif
    ui->ipEdit->setValidator(pReg);
#if QT_VERSION < 0x060000  // < 6.0
    pReg = new QRegExpValidator(regExpNetPort, this);
#else
    pReg = new QRegularExpressionValidator(regExpNetPort, this);
#endif
...
QString TcpUdpPort::autoOpen(bool open)
{
    // 这里目前还没有写实际的函数
    if(open){
        autoOpenPortName=QString::number(protocol);
    }else{
       autoOpenPortName="";
    }
