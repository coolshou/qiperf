...
Placemark::Placemark(const QGV::GeoPos& geoPos, int color)
{
    ...
    if (color == MarkColor::Yellow){
        ...
        sColor=":/placemark/purple";
    }
    // load(QUrl{ "http://maps.google.com/mapfiles/kml/paddle/blu-circle.png" });
    QImage resourceImage(sColor);
    load(resourceImage);
}

void Placemark::load(const QImage &img)
{
    loadImage(img);
}
